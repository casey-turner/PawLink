...
function dog_register() {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $dogPhoto = !empty($_POST['newProfileImage']) ? sanitiseUserInput($_POST['newProfileImage']) : null;

        echo '<pre>';
        var_dump($_POST['newProfileImage']);
        echo "</pre>";
        echo '<pre>';
        var_dump($dogPhoto);
        echo "</pre>";
...
function edit_dog() {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $gender = !empty($_POST['gender']) ? sanitiseUserInput($_POST['gender']) : null;
        $dogDescription = !empty($_POST['dogDescription']) ? sanitiseUserInput($_POST['dogDescription']) : null;
        $dogPhoto = !empty($_POST['newDogProfileImage']) ? sanitiseUserInput($_POST['newDogProfileImage']) : null;
        ...
        if ( !is_null($deleteDogProfileImage) || !is_null($dogPhoto) ) {
            $existingProfileImage = selectData('dogs', array(
                'select' => ' dogProfileImage',
                'where'=> array('dogID' => $dogID ),
                'return type' => 'single'
                )
            );

            if ( !empty($existingProfileImage['dogProfileImage']) ) {
                unlink('view/uploads/'.$existingProfileImage['dogProfileImage']);
                $imageName = '';
            }
        }
